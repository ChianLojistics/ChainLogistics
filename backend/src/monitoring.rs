    ...
    pub async fn record_error(
        ...
        *metrics.error_counts.entry(code_str.clone()).or_insert(0) += 1;
        ...
        if metrics.error_rate > 10.0 {
            tracing::warn!(error_rate = metrics.error_rate, "High error rate detected");
        }
    }
...
impl MonitoringSystem {
    ...
    pub async fn check_alerts(&self) {
        ...
        if self
            .alerter
            .should_alert(ErrorCode::InternalServerError, error_stats.error_rate)
            .await
        {
            self.alerter
                .trigger_alert(format!(
                    "High error rate detected: {:.2} errors/min",
        ...
        if infrastructure.db_pool_usage > 0.9 {
            self.alerter
                .trigger_alert(format!(
                    "High database pool usage: {:.1}%",
                    infrastructure.db_pool_usage * 100.0
                ))
                .await;
        }
        ...
        if !infrastructure.redis_connected {
            self.alerter
                .trigger_alert("Redis connection lost".to_string())
                .await;
        }
    }
}
...
mod tests {
    ...
    async fn test_error_monitor_calculates_rate() {
        ...
        for i in 0..5 {
            monitor
                .record_error(ErrorCode::ValidationFailed, format!("test-{}", i), None)
                .await;
        }
    ...
    async fn test_error_monitor_top_errors() {
        ...
        for _ in 0..3 {
            monitor
                .record_error(ErrorCode::DatabaseError, "test".to_string(), None)
                .await;
        }

        for _ in 0..2 {
            monitor
                .record_error(ErrorCode::ValidationFailed, "test".to_string(), None)
                .await;
        }
