...
/// GET /api/v1/analytics/resilience
pub async fn resilience(
    State(state): State<AppState>,
) -> Result<Json<serde_json::Value>, AppError> {
    let analytics = state.analytics_service.get_resilience_analytics().await?;
    Ok(Json(json!(analytics)))
}

/// GET /api/v1/analytics/fraud
pub async fn fraud(State(state): State<AppState>) -> Result<Json<serde_json::Value>, AppError> {
    let analytics = state.analytics_service.get_fraud_analytics().await?;
    Ok(Json(json!(analytics)))
}
