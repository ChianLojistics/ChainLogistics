use chainlojistic_backend::{
    config::Config, database::Database, models::NewProduct, services::ProductService,
};
use sqlx::PgPool;
use uuid::Uuid;
...
async fn test_database_connection_and_migrations() {
    ...
    config.database.url =
        "postgres://chainlogistics:password@localhost/chainlogistics_test".to_string();
    ...
    // Get product
    let retrieved = product_service
        .get_product("TEST-001")
        .await
    ...
    let updated_result = product_service
        .update_product("TEST-001", updated)
        .await
    ...
    // Delete product
    product_service
        .delete_product("TEST-001")
        .await
        .expect("Failed to delete product");

    // Verify deletion
    let deleted = product_service
        .get_product("TEST-001")
        .await
...
async fn test_product_service_filters() {
    ...
    let results = product_service
        ...
        .await
        .expect("Failed to filter by owner");
